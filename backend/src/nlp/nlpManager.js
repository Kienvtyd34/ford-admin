...
export const processSemanticAI = async (message, memory = {}) => {
  ...
  if (best) {
    entities.variantName = best.variantName;

    // AUTO FIX MODEL FROM VARIANT (CRITICAL)
    if (!entities.modelName && best.modelId) {
      const model = await VehicleModel.findById(best.modelId);
      if (model) entities.modelName = model.name;
    }
  }
  ...
  if (nums?.length) {
    ...
    if (msg.includes("ty")) entities.minBudget = v * 1e9;
    else entities.maxBudget = v * 1e6;
  }
